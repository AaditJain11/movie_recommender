...
filtered_df = df.copy()
if selected_genres:
    filtered_df = filtered_df[filtered_df['genres'].apply(lambda g: any(x in str(g).split('|') for x in selected_genres))]
...
with tab1:
    ...
    if st.button("Get recommendations"):
        if movie not in df['title'].values:
            st.warning("Movie not found. Try adjusting filters.")
            st.stop()
        idx    = df[df['title'] == movie].index[0]
        scores = sorted(enumerate(sim_matrix[idx]), key=lambda x: x[1], reverse=True)
        filtered_indices = set(filtered_df.index)
        scores = [(i, s) for i, s in scores if i in filtered_indices and i != idx][:n]
